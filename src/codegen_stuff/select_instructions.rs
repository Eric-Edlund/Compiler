...
pub fn select_instructions(file: &FileAnal) -> X86Program {
    let mut result = X86Program {blocks: HashMap::new()};

    result.blocks.insert("main".to_string(), Vec::new());
    for construct in &file.asts {
        result.blocks.get_mut("main").unwrap().extend(
            si_stmt(construct)
        );
    }
...
fn si_stmt(exp: &BasedAstNode) -> Vec<X86Instr> {
    ...
    match exp.as_ref() {
        Declaration { identifier, rhs } => {
            ...
            prefix.push(X86Instr::Movq{src: arg, rd: X86Arg::Var{name: identifier.to_string()}});
            prefix
        }
        Assignment { lhs, rhs } => {
            ...
            prepare_value.push(X86Instr::Movq{src: arg, rd: X86Arg::Var{name: identifier.clone()}});
            prepare_value
        }
        ...
        FunctionCall { function, ref args } => {
            ...
            vec![
                X86Instr::Movq{src: args, rd: X86Arg::Reg{name: "rdi".to_string()}},
                X86Instr::Callq{label: "print_int".to_string()},
            ]
        }
        Block {stmts} => {
...
fn si_expr(exp: &BasedAstNode) -> (Vec<X86Instr>, X86Arg) {
    ...
    match exp.as_ref() {
        LiteralNumber(val) => {
            (vec![], X86Arg::Immed{value: val.to_string()})
        }
        Variable { identifier } => {
            (vec![], X86Arg::Var{name: identifier.clone()})
        }
        BinOp { op: BinOperation::Add, ref lhs, ref rhs } => {
            let tmp = X86Arg::Var{name: new_var_name()};
            let (prefix, res) = si_expr(lhs);
            let (prefix2, res2) = si_expr(rhs);
            let mut instrs = vec![];
            instrs.extend(prefix);
            instrs.push(X86Instr::Movq{src: res, rd: tmp.clone()});
            instrs.extend(prefix2);
            instrs.push(X86Instr::Addq{val: res2, rd: tmp.clone()});
            (instrs, tmp)
        }
        _ => panic!("Unimplemented si_expr {:?}", *exp.as_ref())
    }
